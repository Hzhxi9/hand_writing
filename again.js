...
function shallowClone(o) {
    if (typeof o !== "object")
        throw new TypeError(`${o} is on object`);
...
function deepCloneAttr(o) {
    if (typeof o !== "object")
        throw new TypeError(`${o} is no object`);
    const clone = o instanceof Array ? [] : {};
    for (const k in o) {
...
function simpleThrottle(f, delay) {
    let count = 0;
    return function () {
        const ctx = this, args = arguments, now = +new Date();
        if (now - count > delay) {
            f.call(ctx, ...args);
        }
    };
...
function throttle(fn, delay, options) {
    ...
    const throttled = function () {
        ...
        if (!count && options.leading === false)
            count = now;
...
const parserParams = data => {
    if (!isObjectType(data))
        throw new TypeError(data + "is no object");
    let result = "";
...
Array.prototype.writeForEach = function (cb, thisArg) {
    ...
    if (typeof cb !== "function")
        throw new TypeError(`${cb} is no function`);
...
Array.prototype.writeReduce = function (cb, initialValue) {
    ...
    if (arguments.length > 1) {
        ...
        if (k > len)
            throw new TypeError(` ${this} is not empty`);
        acc = o[k++];
    }
    while (k < len) {
        if (k in o) {
...
Function.prototype.writeBind = function (context) {
    const ctx = this, args = Array.prototype.slice.call(arguments, 1);
    function Fun() { }
    function bindFun() {
...
function new_1(f) {
    if (typeof f !== "function")
        throw new TypeError(f + "is not function");
    const o = new Object();
...
Object.writeAssign = function (target, ...source) {
    if (target === null)
        throw new TypeError(target + "is null");
